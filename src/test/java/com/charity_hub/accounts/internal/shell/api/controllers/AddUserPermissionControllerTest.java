...
class AddUserPermissionControllerTest {
    ...
    void shouldAddPermissionToUserSuccessfully() throws Exception {
        ...
        ChangePermissionRequest request = new ChangePermissionRequest("CREATE_CASES", true);
        
        doNothing().when(changePermissionHandler).handle(argThat(cmd ->
                cmd.userId().equals(userId) &&
                cmd.permission().equals("CREATE_CASES") &&
                cmd.shouldAdd()
        ));

        mockMvc.perform(post("/v1/accounts/{userId}/add-permission", userId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk());

        verify(changePermissionHandler).handle(argThat(command ->
